const params=new URLSearchParams(location.search);const key=params.get('for')||'cheso';const base=window.PROSPECTS[key]||window.PROSPECTS.cheso;const requestedLanguage=params.get('lang');const d=requestedLanguage==='en'&&base.en?{...base,...base.en,locale:'en-BI'}:base;
...
if(base.locale==='fr-BI'){
  const switcher=document.createElement('div');switcher.className='language-switcher';const hasEnglish=Boolean(base.en);switcher.innerHTML=`<button class="${d.locale==='fr-BI'?'active':''}" data-lang="fr">Français</button><button data-lang="rn">Kirundi</button><button class="${d.locale==='en-BI'?'active':''}" data-lang="en">English</button>`;document.querySelector('header').insertBefore(switcher,document.querySelector('header nav'));
  ...
  switcher.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;if(b.dataset.lang==='fr'){params.delete('lang');location.search=params.toString();return}if(b.dataset.lang==='en'&&hasEnglish){params.set('lang','en');location.search=params.toString();return}note.textContent=b.dataset.lang==='rn'?(d.locale==='fr-BI'?"La version Kirundi sera activée lors du déploiement final après validation de la traduction par l’équipe de l’organisation.":'The Kirundi version will be activated in the final deployment after the organization validates the translation.'):(d.locale==='fr-BI'?"La version anglaise sera activée après validation de la traduction par l’organisation.":'The English version is currently displayed.');note.classList.add('show');clearTimeout(window.langTimer);window.langTimer=setTimeout(()=>note.classList.remove('show'),4200)});
}
