#!/usr/bin/env python3
"""Append 5 Korvia logo concepts mixing Direction 251 (Negative Cut) and Direction 181 (QR Dissolve)."""
...
QR_PATTERN = '''  <pattern id="qrgrid" x="0" y="0" width="20" height="20" patternUnits="userSpaceOnUse">
    <rect width="20" height="20" fill="#1E3A8A"/>
    <rect x="2" y="2" width="7" height="7" fill="#C0C0C0"/>
    <rect x="11" y="2" width="7" height="7" fill="#C0C0C0"/>
    <rect x="2" y="11" width="7" height="7" fill="#C0C0C0"/>
    <rect x="11" y="11" width="5" height="5" fill="#C0C0C0"/>
  </pattern>'''

LETTER_TEXT = 'font-family="Cinzel, serif" font-size="64" font-weight="700" text-anchor="middle" letter-spacing="0.08em"'
...
CONCEPTS = [
    {
        "id": "259",
        "name": "QR Negative Cut",
        "desc": "The Korvia wordmark is carved out of a solid QR-code block by negative space, leaving only the letters as dark openings.",
        "palette": ["#0A0F1E", "#C0C0C0", "#1E3A8A"],
        "shapes": ["wordmark", "qr", "negative-cut"],
        "mark_css": ".logo-mark svg { max-width: 420px; filter: drop-shadow(0 0 18px rgba(30,58,138,0.45)); }",
        "svg": f'''<svg viewBox="0 0 360 120" xmlns="http://www.w3.org/2000/svg" aria-label="QR Negative Cut">
  <defs>
{QR_PATTERN}
  </defs>
  <rect x="20" y="10" width="320" height="100" fill="url(#qrgrid)"/>
  <text x="180" y="80" {LETTER_TEXT} fill="#0A0F1E" stroke="#C0C0C0" stroke-width="1.5" paint-order="stroke">KORVIA</text>
</svg>''',
    },
    {
        "id": "260",
        "name": "Scan Line Carve",
        "desc": "A horizontal scan line sweeps across a QR block, cutting the Korvia letters out of the modules as it passes.",
        "palette": ["#0A0F1E", "#C0C0C0", "#1E3A8A"],
        "shapes": ["wordmark", "qr", "scan", "animation"],
        "mark_css": ".logo-mark svg { max-width: 420px; filter: drop-shadow(0 0 18px rgba(30,58,138,0.45)); }\n.scan-bar { animation: scanMove 2.2s ease-in-out infinite alternate; }\n@keyframes scanMove { 0% { transform: translateX(-140px); } 100% { transform: translateX(140px); } }\n.logo-mark { overflow: hidden; }",
        "svg": f'''<svg viewBox="0 0 360 120" xmlns="http://www.w3.org/2000/svg" aria-label="Scan Line Carve">
  <defs>
{QR_PATTERN}
    <mask id="scan-cut">
      <rect x="20" y="10" width="320" height="100" fill="white"/>
      <text x="180" y="80" {LETTER_TEXT} fill="black">KORVIA</text>
    </mask>
  </defs>
  <rect x="20" y="10" width="320" height="100" fill="url(#qrgrid)" mask="url(#scan-cut)"/>
  <g class="scan-bar">
    <rect x="178" y="5" width="4" height="110" fill="#C0C0C0" opacity="0.9"/>
  </g>
</svg>''',
    },
    {
        "id": "261",
        "name": "Block Carve Pattern",
        "desc": "Korvia is cut from a solid block, and QR modules show through inside the letterforms themselves.",
        "palette": ["#0A0F1E", "#C0C0C0", "#1E3A8A"],
        "shapes": ["wordmark", "qr", "carve"],
        "mark_css": ".logo-mark svg { max-width: 420px; filter: drop-shadow(0 0 20px rgba(192,192,192,0.35)); }",
        "svg": f'''<svg viewBox="0 0 360 120" xmlns="http://www.w3.org/2000/svg" aria-label="Block Carve Pattern">
  <defs>
{QR_PATTERN}
    <clipPath id="block-letters">
      <text x="180" y="80" {LETTER_TEXT}>KORVIA</text>
    </clipPath>
  </defs>
  <rect x="20" y="10" width="320" height="100" fill="#0A0F1E"/>
  <rect x="20" y="10" width="320" height="100" fill="url(#qrgrid)" clip-path="url(#block-letters)"/>
  <text x="180" y="80" {LETTER_TEXT} fill="none" stroke="#C0C0C0" stroke-width="2">KORVIA</text>
</svg>''',
    },
    {
        "id": "262",
        "name": "Pixel Dissolve",
        "desc": "QR pixels dissolve and drift away, revealing the Korvia wordmark underneath as the scan completes.",
        "palette": ["#0A0F1E", "#C0C0C0", "#1E3A8A"],
        "shapes": ["wordmark", "qr", "dissolve"],
        "mark_css": ".logo-mark svg { max-width: 420px; filter: drop-shadow(0 0 18px rgba(30,58,138,0.4)); }",
        "svg": f'''<svg viewBox="0 0 360 120" xmlns="http://www.w3.org/2000/svg" aria-label="Pixel Dissolve">
  <defs>
{QR_PATTERN}
    <linearGradient id="dissolve-fade" x1="0" y1="0" x2="1" y2="0">
      <stop offset="0%" stop-color="white"/>
      <stop offset="60%" stop-color="white"/>
      <stop offset="100%" stop-color="black"/>
    </linearGradient>
    <mask id="dissolve-mask">
      <rect x="20" y="10" width="320" height="100" fill="url(#dissolve-fade)"/>
    </mask>
  </defs>
  <text x="180" y="80" {LETTER_TEXT} fill="#C0C0C0">KORVIA</text>
  <rect x="20" y="10" width="320" height="100" fill="url(#qrgrid)" mask="url(#dissolve-mask)"/>
</svg>''',
    },
    {
        "id": "263",
        "name": "Module Outline",
        "desc": "Korvia is built entirely from the nested outlines of QR finder modules, with no extra symbols.",
        "palette": ["#0A0F1E", "#C0C0C0", "#1E3A8A"],
        "shapes": ["wordmark", "qr", "finder-outline"],
        "mark_css": ".logo-mark svg { max-width: 420px; filter: drop-shadow(0 0 22px rgba(192,192,192,0.35)); }",
        "svg": f'''<svg viewBox="0 0 360 120" xmlns="http://www.w3.org/2000/svg" aria-label="Module Outline">
  <defs>
    <pattern id="finder-outline" x="0" y="0" width="18" height="18" patternUnits="userSpaceOnUse">
      <rect width="18" height="18" fill="#0A0F1E"/>
      <rect x="2" y="2" width="14" height="14" fill="none" stroke="#C0C0C0" stroke-width="2"/>
      <rect x="6" y="6" width="6" height="6" fill="#C0C0C0"/>
    </pattern>
    <clipPath id="outline-letters">
      <text x="180" y="80" {LETTER_TEXT}>KORVIA</text>
    </clipPath>
  </defs>
  <rect x="20" y="10" width="320" height="100" fill="#0A0F1E"/>
  <rect x="20" y="10" width="320" height="100" fill="url(#finder-outline)" clip-path="url(#outline-letters)"/>
  <text x="180" y="80" {LETTER_TEXT} fill="none" stroke="#C0C0C0" stroke-width="1">KORVIA</text>
</svg>''',
    },
]
...
def main():
    ...
    if marker not in text:
        ...
        return
    insert = "\n".join(build_entry(c, TAGLINE) for c in CONCEPTS).rstrip() + "\n"
    new_text = text.replace(marker, "    },\n" + insert + "]\n\nBASE = \"\"\"")
    target.write_text(new_text, encoding="utf-8")
    print(f"Appended {len(CONCEPTS)} mix 251+181 Korvia concepts.")
